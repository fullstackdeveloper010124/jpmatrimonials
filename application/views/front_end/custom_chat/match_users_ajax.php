<?php
    ...
    if(!empty($match_member_list)){
        foreach ($match_member_list as $key => $value) {
            ...
            if(!empty($member_data) && $member_data != ''){
                ...
                if($check_block_us > 0){
                    $alert_message_block = 'This member is blocked you.';
                }
                ...
                if($alert_message_block != ""){
                    $alert_message_text = 'onclick="return alert('."'".$alert_message_block."'".');"';
                }
                ?>
                <div <?php echo $alert_message_text; ?> class="sidebar-name createConversation pointer" id="match-user-list-<?php echo $value['member_id']; ?>" data-conversation_id="<?php echo $conversation_id; ?>" data-current_member_id="<?php echo $current_member_id; ?>" data-current_member_matri_id="<?php echo $current_member_matri_id; ?>" data-member_id="<?php echo $value['member_id'] ?>" data-matri_id="<?php echo $value['matri_id'] ?>"  data-is_chat="<?php echo ($alert_message_text !="") ? 'No' : 'Yes' ?>">
                    <!-- Pass username and display name to register popup -->
                    <a href="javascript:void(0)">
                        <img src="<?php echo $member_photo_url; ?>" alt="<?php echo $value['matri_id'] ?>">
                        <div class="<?php echo (isset($value['custom_chat_id']) && $value['custom_chat_id'] != '') ? "active-c" : "d-active-c" ?>"></div>
                    </a>
