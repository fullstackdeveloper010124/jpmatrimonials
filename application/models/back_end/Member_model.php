...
class Member_model extends CI_Model {
	...
	function member_list_model($status ='ALL', $page =1,$personal_where='')
	{
		...
		$_REQUEST['member_status_approve'] = 'yes'; // to send only member and approve action for msg and email 2021-07-26
		...
		$_POST['member_approved_status']='active_profile';
	...
	public function display_column_arr($id='',$label_head = 'Members Full Profile')
	{
		...
		$image_arra = array(
			array(
				...
				'inline_style'=>'style="height:218px;width:202px;object-fit:contain"',
			),
			array(
		...
		$field_main_array = array(				
			...
			array(
				...
				'field_array'=>array(
					...
					'income_name'=>array('label'=>'Annual Income'),
			...
			array(
				...
				'field_array'=>array(
					...
					'part_expect'=>array('label'=>'Expectations','class_width'=>' col-lg-6 col-md-6 col-sm-12 col-xs-12 chng-wrap-txt','label_width'=>'3','val_width'=>9),
			...
			array(
				...
				'val_width'=>9,
				'field_array'=>array(
					'part_income_name'=>array('label'=>'Annual Income'),
	...
	function filter_form_array()
	{
		...
		$ele_array = array(
			...
			'income'=>array('label'=>'Annual Income','display_in'=>'1','type'=>'dropdown','relation'=>array('rel_table'=>'annual_income','key_val'=>'id','key_disp'=>'annual_income_name'),'is_multiple'=>'yes','display_placeholder'=>'No','class'=>'chosen-select'),
	...
	{	
		...
		if($insert_id != ''){
			...
			if(isset($get_email_template) && is_array($get_email_template) && $get_email_template!=''){
	...
	/* for mega matrimony*/
	//for redeem coupon code in admin 07-08-2023
	public function check_coupan_admin($plan_id='',$couponcode ='')
	{	
		$return['error'] = 'Some issue, Please try again';
		...
		{		
				...
				if(isset($plan_data['plan_amount']) && $plan_data['plan_amount'] !='')
				{
					...
					$after_discount_plan_amount = $plan_amount - ($plan_amount * $plan_data['offer_per'] / 100);
				}
				$discount_amount = 0;
				$gst_amount = 0;
				$coupan_data = $this->common_model->get_count_data_manual('coupan_code',array('coupan_code'=>$couponcode,'status'=>'APPROVED','is_deleted'=>'No'," active_from <= '$today_date' and expired_on >= '$today_date' "),1,' * ','',0,'',0);
				// $couponCodeUsed = $this->common_model->get_count_data_manual('payments',array('discount_detail'=>$couponcode),0);
				if(isset($coupan_data) && $coupan_data !='' && count($coupan_data) > 0 )
				{
					$discount_amount = $coupan_data['discount_amount'];
					$net_amount = $after_discount_plan_amount - $discount_amount;

					if ($net_amount>0) {
				
						if ($taxApllicable == 'Yes' && $serviceTax > 0) {		
							$gst_amount = ($net_amount * $serviceTax) / 100;
							$net_amount = $net_amount + $gst_amount;
						}
					}
					if($discount_amount > $plan_amount)
					{
						$return['error'] = 'Coupon Code cant redeem for this plan';
					}
					else
					{
						$this->discount_amount_temp = $discount_amount;
						$data_array = array('discount_amount'=>$discount_amount,'coupan_code'=>$couponcode,'plan_id'=>$plan_id,'plan_amount_type'=>$plan_data['plan_amount_type'],'net_amount'=>$net_amount,'gst_amount'=>$gst_amount);
						
						$this->session->set_userdata('coupan_data_reddem',$data_array);
						$return['status'] = 'success';
						$return['coupan_data'] = $data_array;
					}
				}
				else
				{	
		...
		{
			$return['error'] = 'Please enter Coupon Code';
		}
		return $return;
	}

	function update_feature_member($id=""){

		if(isset($id) && $id != ''){
			$data_array = array('fstatus'=>'Unfeatured');
			$this->common_model->update_insert_data_common('register',$data_array,array('id'=>$id),1,'');
		}
	}
}