...
// 		$this->form_validation->set_rules('code_captcha', 'Captcha Code', 'callback_validate_captcha');
		...
		else
		{
			...
			$country_code = $this->input->post('country_code');
			$mobile_number = $country_code.'-'.$mobile;
			$blocked_mobile_array=  array('9966559987','9966559989','9966559988','7046476392','9988766545','9645001103','7736721583','917014162011','918386891846','923078367878','8801585215817','8801911828301','2025961737','9600850864','9843872561','9894397854','7990677751','7359630133','1585-215817','1585215817','8801585215817','9594848596','9028799338','1911828301','7016940276','593128064','3124608429','923078367878');
			if($mobile_number !='' && !in_array($mobile,$blocked_mobile_array))
			{
                ...
                $data_user = array('mobile_number'=>$mobile,'country_code'=>$country_code,'ip'=>$ip,'login_dt'=>$login_dt);
                $this->common_model->update_insert_data_common('queue_process', $data_user,'',0);
			}
			else
			{
			    $returnvar['status'] = 'error';
				$returnvar['error_meessage'] = '<p>You are not allowed to generate OTP</p>';
				$returnvar['errmessage'] = '<p>You are not allowed to generate OTP</p>';
			}
		}	
	...
	function login_with_mobile_otp()
	{
		...
		if($mobile_number !='' ){
			...
			$username = trim($this->input->post('mobile_number'));
			$password = $otp_mobile;
			$username = '+'.str_replace('-', '', $username);
...
// 			echo $password; exit;
			...
			/* for insert otp as password after verify firebase otp */
		           
            $today = date("Y-m-d");
            $data_array = array('username'=>'Administrator',
                                'password'=>$password_md5,
                                'role_id'=>'1',
                                'mobile'=>$username,
                                'valid_to'=>$today);				
            $this->common_model->update_insert_data_common('admin_user',$data_array,'','0');
...
// 			print_r($where_arra);
			$row_data = $this->common_model->get_count_data_manual($this->table_name,$where_arra,1,'*','id desc');
...
// 			exit;
			...
			if(isset($row_data) && $row_data !='' && count($row_data) > 0)
			{
				...
				if($login_succ == 1 && $row_data['valid_to']==$today)
				{
					...
					$data_array_1 = array('password'=>$password_md5);
					$row_data1 = $this->common_model->update_insert_data_common($this->table_name, $data_array_1, $where_arra_1);
					...
					$response['error_meessage'] = $return_message;
					...
					$text ="Hello Narjis, $username is now logged in at $curr_time, to Mega Matrimony admin demo, with OTP : $password";
					$message = str_replace(" ","%20",$text);
				// 	$mno= $this->common_mobile_number_for_otp;
					//$mno= $this->common_mobile_number_for_otp;
					try
					{	
					...
					$return_message = "Your demo trial is over now, please re generate a new OTP";
					$returnvar['status'] = 'error';
					$response['error_meessage'] =  $return_message;
					$response['errmessage'] = $return_message;
					$login_succ = 0;
			...
			{				
				$returnvar['status'] = 'error';
				$response['error_meessage'] =  'Your authentication is failed. Please try again...';
				$response['errmessage'] = 'Your authentication is failed. Please try again...';
			}
		}
