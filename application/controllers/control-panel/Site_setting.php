...
RewriteRule ^(.*\.(gif|jpeg|jpg|png))$ ".base_url()."assets/photos/watermark.php?image=$1 [NC] ";
					...
					$file = fopen($photo_htacc,"w+");
					$photo1_ht_content = "RewriteEngine on
RewriteRule ^(.*\.(gif|jpeg|jpg|png))$ ".base_url()."assets/photos_big/watermark.php?image=$1 [NC] ";
					fwrite($file,$photo1_ht_content);
					fclose($file);
			// for create or update htaccess for file path
	...
	public function basic_setting($status ='')
	{
		...
		else
		{
			$ele_array = array(
				...
				'near_by_me_km'=>array('is_required'=>'required','input_type'=>'number','label'=>'Near By Me Km (For Match)'),
				'default_country_code'=>array('is_required'=>'required','class'=>' not_reset ','label' => 'Default Country Code','type'=>'dropdown','relation'=>array('rel_table'=>'country_master','key_val'=>'country_code','key_disp'=>'country_code')),	// for relation dropdown
				'vendor_plan_currency'=>array('is_required'=>'required','type'=>'dropdown','label'=>'Plan Currency','relation'=>array('rel_table'=>'currency_master','key_val'=>'currency_code','key_disp'=>'currency_name')),
				'vendor_plan_amount'=>array('is_required'=>'required','input_type'=>'number'),
			);
			$this->common_model->extra_js[] = 'vendor/ckeditor/ckeditor.js';
			$this->common_model->js_extra_code = " if($('#full_address').length > 0) {  $('.website_keywords_edit').removeClass(' col-lg-9 ');
				$('.website_keywords_edit').addClass(' col-lg-7 ');
...
';
	...
	public function notification_setting($status ='')
	{
		$this->label_page = 'Update Basic Site Settings';
		if(isset($status) && $status == 'save-data'){
			$this->common_model->save_update_data(1,'','Yes');
			...
			$ele_array = array(
				//'notification_access_key'=>array('is_required'=>'required'),
				'app_click_action'=>array('is_required'=>'required'),
				'notification_script'=>array('type'=>'textarea','is_required'=>'required'),
				'vapid_key'=>array('is_required'=>'required','label'=>'VAP ID'),
				'is_firebase_otp'=>array('type'=>'radio','value_arr'=>array('Yes'=>'Yes','No'=>'No')),
			);
			$other_config = array('mode'=>'edit','id'=>'1');
			$this->data['data'] = $this->common_model->generate_form_main($ele_array,$other_config);
	...
	public function site_image($status ='')
	{
		$this->label_page = 'Update Site Image';
		if(isset($status) && $status == 'save-data')
		{	
			$this->common_model->save_update_data(1,'','Yes');
		}
		else
		{
			$this->common_model->extra_js[] = 'vendor/jquery-validation/dist/additional-methods.min.js';
			$ele_array = array(
				'male_placeholder'=>array('type'=>'file','path_value'=>'assets/front_end/img/default-photo/','display_note'=>'File size must be 512px * 620px'),
				'female_placeholder'=>array('type'=>'file','path_value'=>'assets/front_end/img/default-photo/','display_note'=>'File size must be 512px * 620px'),
				'male_protected_placeholder'=>array('type'=>'file','path_value'=>'assets/images/','display_note'=>'File size must be 512px * 620px'),
				'female_protected_placeholder'=>array('type'=>'file','path_value'=>'assets/images/','display_note'=>'File size must be 512px * 620px'),
				'login_page_banner'=>array('type'=>'file','path_value'=>'assets/front_end_new/images/','display_note'=>'File size must be 1366px * 589px'),
				'site_back_image'=>array('type'=>'file','path_value'=>'assets/front_end_new/images/','display_note'=>'File size must be 512px * 620px'),
			);
			$other_config = array('mode'=>'edit','id'=>'1','enctype'=>'enctype="multipart/form-data"');
			$this->data['data'] = $this->common_model->generate_form_main($ele_array,$other_config);
